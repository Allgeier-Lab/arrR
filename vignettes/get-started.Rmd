...
reef_matrix <- matrix(data = c(-15, 0, 0, 15, 15, 0, 0, -15, 0, 0), 
                      ncol = 2, byrow = TRUE)
...
# create fishpop
input_fishpop <- setup_fishpop(seafloor = input_seafloor, species = c(2, 2, 2, 1, 1, 1), 
                               starting_values = starting_values, 
                               parameters = parameters)

#changing parameters for species 2, sets reserves thresh to 0
#something about this is throwing the reserves off --> used in simulate fn
parameters$pop_reserves_thres_mean[2] <- 0

#subsequent edits are for other parameters (mostly to test locally)
#can have reserves after first life if param below are not run
#should still look at the behavior tree

parameters$pop_a[2] <- 0.05
parameters$pop_b[2] <- 2.9
parameters$pop_k[2] <- 0.1
parameters$pop_linf[2] <- 39
parameters$pop_n_body[2] <- 0.015
parameters$resp_intercept[2] <- 0.1
parameters$resp_slope[2] <- -0.18
...
# make sure species 2 is within 1 unit of a reef
```

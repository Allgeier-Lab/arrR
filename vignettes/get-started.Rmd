...
# create fishpop
input_fishpop <- setup_fishpop(seafloor = input_seafloor, species = c(2, 1), 
                               starting_values = starting_values, 
                               parameters = parameters)

#changing parameters for species 2, sets reserves thresh to 0
for(i in 1:nrow(input_fishpop)) {
  if (input_fishpop[i,2] == 2) {
    parameters$pop_reserves_thres_mean[i] <- 0
    #subsequent edits are for other parameters (mostly to test locally)
    parameters$pop_a[i] <- 0.2
    parameters$pop_b[i] <- 3.5
    parameters$pop_k[i] <- 0.3
    parameters$pop_linf[i] <- 44
    parameters$pop_n_body[i] <- 0.05
    parameters$resp_intercept[i] <- 0.11
    parameters$resp_slope[i] <- -0.22
  }
}

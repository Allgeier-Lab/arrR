...
3) Individuals move randomly (see "Random movement" above) and forage until their energy reserves are filled completely. 
...
1) Individuals are considered to be "on the reef" if their distance to the closest reef cell is less than `move_border`. Individuals move randomly using a distance sampled from a log-normal distribution using `move_reef` as mean until their energy reserves are below `pop_reserves_thres`. During this behavior, individuals do not forage.

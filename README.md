...
`setup_environment` and `setup_population`. If you want to add artifical
reefs to the environment, provide a `matrix` with x,y coordinates of all
AR cells.
...
input_population <- setup_population(environment = input_environment, 
                                     starting_values = starting_values, parameters = parameters)
#> Creating 100 individuals within extent(-50, 50, -50, 50)...
```
